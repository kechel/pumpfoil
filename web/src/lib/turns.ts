...
// Kurvenlage-g -> Farbe. g=0 = kein Carve (grau). Sonst Verlauf grün (0,1 g) →
// gelb (0,35 g) → rot (≥0,6 g) — realistischer Pumpfoil-Bereich (~0–0,6 g).
export function carveColor(g: number): string {
  if (g <= 0.02) return "#334155";
  const gc = Math.max(0.1, Math.min(g, 0.6));
  return gc <= 0.35 ? lerpHex("#22c55e", "#eab308", (gc - 0.1) / 0.25)
                    : lerpHex("#eab308", "#dc2626", (gc - 0.35) / 0.25);
}
