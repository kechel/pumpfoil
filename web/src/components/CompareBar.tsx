...
import { useCompare, mergeableIds } from "../lib/compare";
import { useT } from "../i18n";

// Schwebender Button: erscheint, sobald etwas im Vergleichskorb liegt, und führt
// zur Vergleichsansicht. Über der Mobile-Bottom-Nav positioniert. Sind nur eigene
// Sessions gleichen Datums im Korb, heisst er „Vergleichen und Mergen".
export function CompareBar() {
  const t = useT();
  const refs = useCompare();
  if (refs.length < 1) return null;
  const canMerge = mergeableIds(refs) != null;
  return (
    <Link
      to="/vergleich"
