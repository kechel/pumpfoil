...
const ENTRIES: Entry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "One thing in that version is broken, and it is worth knowing before you tap it: videos in the community feed stay black. YouTube tightened how it checks which page a player is embedded in, and ours no longer passed \u2014 the sound plays, the picture does not. It is fixed and comes with the next update, on both the iPhone and Android. Until then the title of the clip and the button to YouTube still work, so you can watch it there.",
      "Android and Wear OS 1.1.24 / 1.2.24 are live in the Play Store. On the phone: value graphics in the layout preview and heart-rate zones in your profile, GPX and FIT download, spot descriptions, the water name next to the spot, aspect-ratio badges, a catalogue search that ignores word order, and the training curve. On the watch: an always-on view, BACK no longer drops out of a running recording, a live distance that stops counting when you stand still, a cleaned-up top speed, and runs that belong together are no longer split.",
  ...
  },
  {
    ...
    items: [
      ...
      "Community records and the leaderboards can now be narrowed to comparable foils. There is a new picker next to the time range, and the entry worth trying is \u201clike my foil\u201d: it takes your own wing and shows only records set on wings within 15\u202f% of its area and 2 points of its aspect ratio. We checked whether area alone would do, and it does not \u2014 among the wings ridden here, area tells you almost nothing about aspect ratio, and inside one size class the aspect ratio ranges from 4 to 20. A 1500 that glides and a 1450 that lifts early are simply not the same tool. There are also plain size classes and a high-aspect group, and each option says how many sessions are behind it, so you can see when a group gets too thin to mean much.",
      "The session lists start on \u201call\u201d now instead of \u201cprecise only\u201d, even if your own watch records motion data. The old default quietly hid the sessions of everyone whose watch does not, which is how one rider ended up staring at three sessions on a spot the map said had fourteen. Records and leaderboards still default to the precise ones, where that accuracy is the point.",
      "The progress animation at a spot no longer draws odd spikes across the water. It used to draw the whole recording thinned down to 150 points, so on a long session it connected places you had been fourteen seconds apart \u2014 which looks exactly like a broken GPS. It now draws each run on its own, and the same 150 points buy full detail on the parts where you were actually riding.",
      "If you have ever merged two recordings into one session, its times are right again. The merge was stitching the parts together back-to-back instead of placing them where they actually happened, so runs could be shown up to hours off \u2014 in one case a rider and his mate rode the same line together and the app put them sixteen minutes apart. All 48 merged sessions in the database were rebuilt; distances, speeds and pump counts were never affected, and six of them even gained runs that the old stitching had cut in half.",
      "The app now runs on the f\u0113nix 9, 9 Pro and 9 Pro Solar \u2014 and on the f\u0113nix 8 in 43\u202fmm, which we had been missing without noticing. That makes 129 supported Garmin watches. If you already have a Garmin, this update changes nothing for you, so there is no need to hurry.",
      "New in the gear list, and every one of them because a rider asked: the Levitaz Free Series Prototype 1200, the AlpineFoil DK 1360, and two tails we had been missing \u2014 the AlpineFoil HA 175 and the Levitaz Stabilizer 180. The Levitaz 1200 is why nobody could find it: it is a prototype, so it appears in no spec sheet anywhere, and it is in the list now with the figures its rider gave us and \u201cPrototype\u201d in the name, because the gear list is shared by everyone and this is not a wing you can buy. Its numbers work out to an aspect ratio of 18.8 rather than the 18.1 that gets quoted, which usually means the maker measures the area a little differently \u2014 we store the two measured figures and let the ratio follow from them. All four can be found under the names people actually type as well, so \u201cFreeSeries 1200\u201d or \u201cHA175\u201d gets you there.",
      "Fixed: on a narrow phone a few of the pickers could push the whole page sideways, so you had to scroll left and right to read it. The spot picker above the progress animation was the worst of them \u2014 a long spot name simply made the page wider than the screen. Those pickers now stay inside the screen and shorten the name instead, and we went through the other pages for the same pattern rather than fixing just the one that was reported.",
      "Coming with the next update of the watch apps: the Apple Watch and Wear OS now switch GPS on while you are still on the start screen and tell you when it is ready. Until now they only started looking for satellites when you pressed start, and a cold start takes a good two minutes outdoors \u2014 in one measured session only 15 positions arrived in the first 132 seconds, and the run in that window is gone for good. Garmin, Zepp and the phone apps have warmed GPS up all along.",
    ],
  },
  {
    date: "August 31, 2026",
    items: [
      "The iPhone and Apple Watch app has caught up with everything the website gained this month \u2014 version 1.1.27, in the App Store now. Your records and totals now come once for every foil you ride, the tiles start on the last ten days, all five maps have the satellite switch, the community video feed is there with full screen and skip-through, and you can attach a screenshot or a log to feedback. Four things Jan found while testing are fixed too: the spots map zooms again and tapping empty water no longer opens some random spot, the video full screen stays open on the first tap, and preview images no longer vanish when you switch tabs and come back.",
      "The apps also speak all seventeen languages properly now. Until this version, between 45 and 183 pieces of text per language quietly fell back to English \u2014 you would set the app to Czech or Finnish and still meet English buttons halfway through. Dutch, Finnish and Czech are new on the Apple Watch as well. Nothing was machine-translated for this: every line comes from the same wording the website already used.",
