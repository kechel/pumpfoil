...
|---|---|---|---|
| 2026-09-21 (Freigabe) | iOS + Apple Watch (App Store) | 1.1.36 | ✅ **FREIGEGEBEN UND LIVE.** ZWEITE Apple-Mail („The following app is ready for distribution · App Version Number: 1.1.36 · Platform: iOS"). **Nicht nur die Mail geglaubt:** die Store-API meldet in **de/us/gb/ch** uebereinstimmend 1.1.36 mit `currentVersionReleaseDate` 2026-09-21T02:28:55Z (04:28 Berlin) — diesmal ohne den Cache-Nachhang, den 1.1.35 und 1.1.31 noch hatten. Inhalt: der Handy-Recorder zeichnet zusaetzlich das Gyroskop auf, falls vorhanden. `appmeta` auf 1.1.36 gesetzt (beide Schluessel `ios` und `apple`, ein Bundle), der NAECHSTES-Eintrag raus, Changelog-Punkt mit `versionen={ios,apple}` und ein Freigabe-Ereignis eingetragen. **EINGEREICHT am Abend des 20.09., Upload auf die Minute belegt.** Apples Mail „The following build has completed processing · Build Number: 40 · Version Number: 1.1.36" kam **19:05**; gebaut aus Commit `b8ec06f1` (19:02:40), davor `99df0974` 18:55:44 „iOS auf 1.1.36 / Build 40 gebumpt". Die Einreichung zur Pruefung liegt danach — Apple laesst erst nach der Verarbeitung einreichen —, die Freigabe bei 04:28:55 am Folgetag: **gut neun Stunden Pruefung**, passend zu 1.1.33 (11 h). Unbelegt bleibt nur die Spanne zwischen fertiger Verarbeitung und dem Einreichen-Klick, also Minuten. (Erst als Schaetzwert eingetragen, dann durch Jans Verarbeitungs-Mail ersetzt — die Schaetzung hatte ~19:10 angenommen und lag damit richtig.) |
